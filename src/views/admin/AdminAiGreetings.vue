<template>
  <div class="admin-ai-greetings">
    <el-card class="block" shadow="never">
      <div class="toolbar">
        <div class="toolbar-right">
          <el-button type="primary" @click="onAdd">新增问候语</el-button>
    ...
    <el-card class="block" shadow="never">
      <el-form :inline="true" :model="query" class="search-form">
        <el-form-item label="关键字">
          <el-input v-model="query.keyword" placeholder="问候语内容" clearable style="width: 200px" />
        </el-form-item>
    ...
    <el-card class="block" shadow="never">
      <div class="table-wrap">
        <el-table :data="list" border style="width: 100%" v-loading="loading" @selection-change="onSelectionChange" @sort-change="onSortChange">
          ...
          <el-table-column prop="content" label="问候语内容" min-width="320" show-overflow-tooltip />
        ...
        <el-form ref="formRef" :model="form" :rules="rules" :disabled="dialog.mode === 'view'" label-width="96px" class="dialog-form">
          <div class="row row-2">
            <el-form-item label="内容" class="col col-2" prop="content" required>
              <el-input v-model="form.content" placeholder="请输入问候语内容" type="textarea" :rows="3" />
            </el-form-item>
          </div>
          <div class="row row-2">
...
import {
  fetchAdminAiGreetingsPage,
  fetchAdminAiGreetingById,
  createAdminAiGreeting,
  updateAdminAiGreeting,
  deleteAdminAiGreeting,
} from '@/api'
...
const dialogTitle = computed(() => {
  if (dialog.mode === 'create') return '新增问候语'
  if (dialog.mode === 'edit') return '编辑问候语'
  return '问候语详情'
})
...
async function loadData() {
  loading.value = true
  try {
    const data = await fetchAdminAiGreetingsPage(buildPagePayload())
...
async function loadDetail(id) {
  try {
    const data = await fetchAdminAiGreetingById(id)
    fillForm(data || {})
  } catch (e) {
...
async function onDelete(row) {
  try {
    await ElMessageBox.confirm(`确认删除该问候语？`, '提示', { type: 'warning' })
    await deleteAdminAiGreeting(row.id)
...
async function onBatchDelete() {
  ...
  try {
    ...
    for (const id of ids) {
      await deleteAdminAiGreeting(id)
    }
